...
```mermaid
graph TD
    A[Express API] --> B(Blockchain Core)
    B --> C{UTXO Pool}
    B --> D[(LevelDB Persistence)]
    E[P2P WebSockets] <--> B
    E <--> F[Transaction Mempool]
    G[Wallet] --> H[Secp256k1 Cryptography]
    G --> F
```
...
Once your nodes are running, you can use `cURL` or Postman to interact with the HTTP APIs.
...
**3. Mine a Block (Processes Mempool & Awards Coinbase)**
```bash
